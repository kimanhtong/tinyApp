...
const generateRandomString = (length) => {
  ...
  for (let i = 0; i < length; i++) {
    const charIndex = Math.floor(62 * Math.random());
...
const emailVal = (email) => { return email !== '' && email !== null && email !== undefined };

// Function to validate empty password
const passwordVal = (password) => { return password !== '' && password !== null && password !== undefined };
...
const urlsForUser = (email, users, urlDatabase) => {
...
const urlAuthorized = (email, shortURL, users, urlDatabase) => {
  ...
  const urls = urlsForUser(email, users, urlDatabase); // get the short-long URL pairs which user can access
